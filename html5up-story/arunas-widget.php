...
// Valdiklio išvestis - kas matoma lankytojui frontende. 
  public function widget( $args, $instance ) {
    ...
    <div class="caption">
      ...
      <ul class="actions">
        <li><span class="button small"><?php _e('Details', 'html5up-story'); ?></span></li>
      </ul>
    </div>
  ...
  public function form( $instance ) {
    if ( isset( $instance[ 'title' ] ) ) {
      ...
      $title = __( 'New title', 'html5up-story' );
    }
    echo '<p>';
    echo '<label for="' . $this->get_field_id( 'title' ) . '">' . __( 'Title:', 'html5up-story' ) . '</label>';
    echo '<input class="widefat" id="' . $this->get_field_id( 'title' ) . '" name="' . $this->get_field_name( 'title' ) . '" type="text" value="' . esc_attr( $title ) . '" />';
   
       if ( isset( $instance[ 'description' ] ) ) {
      $description = $instance[ 'description' ];
    } else {
      $description = __( 'New description', 'html5up-story' );
    }
     echo '<p>';
    echo '<label for="' . $this->get_field_id( 'description' ) . '">' . __( 'Description:', 'html5up-story' ) . '</label>';
    ...
    echo '</p>';
      ...
      $image = __( 'New image', 'html5up-story' );
    }
     echo '<p>';
    echo '<label for="' . $this->get_field_id( 'image' ) . '">' . __( 'Image:', 'html5up-story' ) . '</label>';
